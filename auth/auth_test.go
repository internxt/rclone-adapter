...
func TestRefreshToken(t *testing.T) {
	testCases := []struct {
		...
		},
		{
			...
			expectError:    true,
			errorContains:  "refresh token: error message (status 401)",
		},
		{
			...
			expectError:    true,
			errorContains:  "refresh token: error message (status 500)",
		},
		{
			name:           "forbidden - 403",
			token:          "valid-token",
			...
			expectError:    true,
			errorContains:  "refresh token: error message (status 403)",
		},
		{
