...
// TestUploadFileStream_RetriesCreateMetaOn404 verifies that UploadFileStream
// retries the CreateMetaFile call when the API returns 404 (eventual consistency).
func TestUploadFileStream_RetriesCreateMetaOn404(t *testing.T) {
	mockServer := newMockMultiEndpointServer()
	defer mockServer.Close()
	mockServer.SetupSuccessfulUploadMock()

	var metaCallCount int32
	failCount := int32(2) // fail first 2 meta calls with 404

	mockServer.createMetaHandler = func(w http.ResponseWriter, r *http.Request) {
		metaCallCount++
		if metaCallCount <= failCount {
			w.WriteHeader(http.StatusNotFound)
	...
	data := bytes.NewReader([]byte("test file content for retry"))
	result, err := UploadFileStream(context.Background(), cfg, TestFolderUUID, "retry-test.txt", data, int64(len("test file content for retry")), time.Now())
	...
	if result.UUID != TestFileUUID {
		t.Errorf("expected UUID %s, got %s", TestFileUUID, result.UUID)
	}
	if metaCallCount != failCount+1 {
		t.Errorf("expected %d meta calls (2 retries + 1 success), got %d", failCount+1, metaCallCount)
	}
}

// TestUploadFileStreamAuto_EmptyFile_RetriesCreateMetaOn404 verifies that the
// empty file path in UploadFileStreamAuto retries CreateMetaFile on 404.
func TestUploadFileStreamAuto_EmptyFile_RetriesCreateMetaOn404(t *testing.T) {
	mockServer := newMockMultiEndpointServer()
	defer mockServer.Close()
	...
	mockServer.createMetaHandler = func(w http.ResponseWriter, r *http.Request) {
		...
		resp := CreateMetaResponse{UUID: "empty-retry-uuid", FileID: ""}
		w.WriteHeader(http.StatusOK)
		json.NewEncoder(w).Encode(resp)
	}

	cfg := newTestConfig(mockServer.URL())
	result, err := UploadFileStreamAuto(context.Background(), cfg, TestFolderUUID, "empty-retry.txt", bytes.NewReader([]byte{}), 0, time.Now())
	...
	if metaCallCount != 2 {
		t.Errorf("expected 2 meta calls (1 retry + 1 success), got %d", metaCallCount)
	}
}

// TestUploadFile_RetriesCreateMetaOn404 verifies that UploadFile
// retries CreateMetaFile on 404 (eventual consistency).
func TestUploadFile_RetriesCreateMetaOn404(t *testing.T) {
	// Create temp file
	tmpDir := t.TempDir()
	testFilePath := filepath.Join(tmpDir, "retry-test.txt")
	if err := os.WriteFile(testFilePath, []byte("retry test content"), 0644); err != nil {
		t.Fatalf("failed to create test file: %v", err)
	}
	...
	cfg := newTestConfig(mockServer.URL())
	result, err := UploadFile(context.Background(), cfg, testFilePath, TestFolderUUID, time.Now())
	...
	if metaCallCount != 3 {
		t.Errorf("expected 3 meta calls (2 retries + 1 success), got %d", metaCallCount)
	}
}
